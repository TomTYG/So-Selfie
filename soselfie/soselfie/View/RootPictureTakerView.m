...
@interface RootPictureTakerView() {
    UIView *cameraView;
...
@implementation RootPictureTakerView
...
-(void)takepicturebuttonpressed:(id)sender {
    //the delegate function for the video object checks whether self.takingphoto is true, and if it is, it creates an image of the camera input. Setting this variable to true basically means "take a picture of the current output the next time you get picture data."
    
    self.takingphoto = true;
    takepicturebutton.enabled = NO;
    
    //this stops the connection link with the videolayer, so that it wont get updated while the app is taking the picture.
